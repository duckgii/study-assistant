...
export default function ChatWidget({ contextTitle, contextPages }: ChatWidgetProps) {
  ...
  return (
    <>
      <button className="fixed bottom-6 right-6 rounded-full bg-blue-600 px-5 py-3 text-sm font-semibold text-white shadow-lg" onClick={() => setOpen((value) => !value)}>
        {t("chat.askAi")}
      </button>

      {open && (
        <div className="fixed bottom-24 right-6 w-[360px] rounded-3xl border border-slate-200 bg-white p-4 shadow-xl">
